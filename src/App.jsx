...
const TABLE_COLS = [
  ...
  { key:'selisih',           label:'Selisih',          align:'right',  w:72  },
...
export default function App(){
  ...
  const exportExcel = ()=>{
    const wsData=[TABLE_COLS.map(c=>c.label),
      ...filtered.map((d,i)=>TABLE_COLS.map(c=>{
        ...
        if(c.key==='selisih'){
          const hasData = d.lulus!==''&&d.tidak_lulus!==''&&d.tms!==''
          if(!hasData) return ''
          return d.total_peserta - ((d.lulus||0)+(d.tidak_lulus||0)+(d.tms||0))
        }
        return d[c.key]??''
      }))]
  ...
  const renderCell = (d, col, rowIdx) => {
    ...
    switch(col.key){
      ...
      case 'selisih': {
        const hasData = !isPlaceholder(d.lulus) && !isPlaceholder(d.tidak_lulus) && !isPlaceholder(d.tms)
        if (!hasData) return <PlaceholderCell th={th}/>
        const realisasi = (d.lulus||0) + (d.tidak_lulus||0) + (d.tms||0)
        const selisih   = d.total_peserta - realisasi
        const color = selisih > 0 ? '#059669' : selisih < 0 ? th.peakColor : th.textMuted
        return (
          <div style={{lineHeight:1.3}}>
            <div style={{color,fontWeight:700,fontSize:13}}>{selisih > 0 ? '+' : ''}{selisih.toLocaleString('id')}</div>
            <div style={{fontSize:10,color:th.textMuted}}>{realisasi.toLocaleString('id')} real.</div>
          </div>
        )
      }
      case 'lulus':
            ...
            <div style={{display:'flex',alignItems:'center',gap:12}}>
              ...
              <div>
                ...
                <div style={{fontSize:11,color:th.textMuted,marginTop:1}}>
                  ...
                  <span style={{color:th.gold,fontWeight:600}}>Selisih = Peserta − (L+TL+TMS) · kosong jika data belum tersedia</span>
                </div>
              </div>
            </div>
